# Bugs and missing
- Geoshell are breaking the shapes in Unity (looks like the character has a ghost hallowing costume, and it doesn't render). Daz is running with one of them (Nadine 9 has G9WT Wet Geoshell Map WB6 High Density Drops geoshell)
- Starting screen (needs an option menu, connection informations and different tools if we have)
...
- Draft mode: this will stop commits to be sent to daz and stop sync. Once definitive, it will commit the poses and resync with Daz

- Start render on Cam: the Unity app will probably need to enter draft mode after this to be able to continue to work as Daz block edits when rendering
